...
fuzz_target!(|data: &[u8]| {
    ...
    let proof_id_hash = match BytesN::<32>::try_from(data[0..32].to_vec()) {
        Ok(h) => h,
        Err(_) => return,
    };

    // Parse commitment_hash (next 32 bytes)
    let commitment_hash = match BytesN::<32>::try_from(data[32..64].to_vec()) {
        Ok(h) => h,
        Err(_) => return,
    };

    // Create issuer_address from data
    let issuer_address = Address::Account(env.crypto().keccak256(&data[64..]).into());
